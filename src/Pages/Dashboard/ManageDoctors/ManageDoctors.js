...
const ManageDoctors = () => {
    ...
    const { data: doctors = [], refetch } = useQuery({
        queryKey: ['doctors'],
        queryFn: async () => {
            const res = await fetch('https://doctors-portal-server-eight-lilac.vercel.app/doctors', {
                headers: {
    ...
    const handleDeletingDoctor = doctor => {
        fetch(`https://doctors-portal-server-eight-lilac.vercel.app/doctors/${doctor._id}`, {
            method: "DELETE",
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
